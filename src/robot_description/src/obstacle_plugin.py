...
class PersonObstaclePlugin:
    ...
    def model_states_callback(self, data):
        ...
        # Get the pose of the person model
        #person_pose = data.pose[person_idx]

    def update(self):
        # Generate random linear and angular velocities with noise
        linear_vel_x = random.uniform(0.5, 2.0) + random.uniform(-0.1, 0.1)  
        angular_vel_z = random.uniform(-1.0, 1.0)  

        # Create the Twist message and set the velocities
        twist_msg = Twist()
        ...
        # Publish the ModelState message to update the person model's velocity
